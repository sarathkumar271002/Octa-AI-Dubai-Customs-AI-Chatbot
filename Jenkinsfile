pipeline {
    ...
    environment {
        PROJECT_DIR = "/var/www/html/Ai-training-platform"
    }
        ...
        stage('Fix Initial Permissions') {
            steps {
                script {
                    ...
                    sudo chown -R jenkins:www-data ${PROJECT_DIR} || true
                    '''
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'git_credentials', 
                        ...
                        sh """
                            cd ${PROJECT_DIR}
                            git remote add origin https://\${GIT_USERNAME}:\${GIT_PASSWORD}@github.com/finloge/Ai-training-platform.git || true
                            git remote set-url origin https://\${GIT_USERNAME}:\${GIT_PASSWORD}@github.com/finloge/Ai-training-platform.git
                            git fetch origin
        ...
        stage('Run Deployment Scripts') {
            steps {
                script {
                    ...
                    sh """
                        cd ${PROJECT_DIR}
                        for script in env.sh gunicorn.sh nginx.sh; do 
                          if [ -f "./\$script" ]; then 
                            echo "Running \$script..."
    ...
    post {
        always {
            script {
                ...
                sudo chmod -R 777 ${PROJECT_DIR} || true
                sudo chown -R jenkins:www-data ${PROJECT_DIR} || true
                sudo chmod -R 777 ${PROJECT_DIR}/storage || true
                sudo chown -R jenkins:www-data ${PROJECT_DIR}/storage || true
                '''
            }
            script {
